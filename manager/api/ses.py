...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    # Accept region as a plain string from frontend
    ...
    def create(self, validated_data):
        ...
        # Handle region by name
        region_name = validated_data.pop("region", None)
        region_instance = None
        if region_name:
            region_instance = Region.objects.filter(region__iexact=region_name).first()
            if not region_instance:
                raise serializers.ValidationError({"region": f"Region '{region_name}' does not exist"})
        ...
        # Assign region if available
        if region_instance:
            user.region = region_instance

        user.save()

        # Assign academic qualifications
